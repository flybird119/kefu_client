...
export default {
  ...
  methods: {
      ...
      IM.init({
        ...
        if(!user){
          ...
          IM.login(() => {
            setTimeout(()=> {
              // 获取本地缓存消息
              this.messages = this.getLocalMessages() 
              this.isLoading = false
    ...
    loadMorData(){
      ...
      setTimeout(()=> {
        var msg = this.getLocalMessages()
        if(msg.length == 0){
          this.isLoadMorEnd = true
        }
    ...
    receiveP2PMsg(message){
      ...
      try{
        if(message.biz_type == "transfer"){
          message.payload = "客服转接"
        }
        if(message.biz_type == "cancel"){
          message.payload = "对方撤回了一条消息"
        }
        if(message.biz_type == "end" || message.biz_type == "timeout"){
          message.payload = "会话超时"
        }
        if(!Push.Permission.has()) return
        Push.create("收到一条新消息", {
            body: message.payload,
            icon: 'http://qiniu.cmp520.com/12814992065748104.jpg',
            timeout: 5000,
            onClick: () => {
              window.focus();
            }
    ...
    messagesPushMemoryAndLocalStoreSave(msg, isPushMemory = true){
      ...
      if(msg.biz_type == 'welcome' || msg.payload == "http://qiniu.cmp520.com/wechat_qrcode.jpeg") return;
      let _messages = []
    ...
    // 处理头像昵称
    handlerMessage(msg){
      const defaultAvatar = 'http://qiniu.cmp520.com/avatar_default.png'
    ...
    getLocalMessages(){
      const pageSize = 15
      let messages = []
      let messagesString = localStorage.getItem("miniImAppMessageRecord_" + this.userInfo.id)
      if(messagesString){
        let messageAll = JSON.parse(messagesString)
        if(this.isFirstGetMessage){
          this.isFirstGetMessage = false
          messages = messageAll.slice(-pageSize, messageAll.length)
        }else{
          var lastIndex = -1
          for(let i = messageAll.length - 1; i >= 0; i--){
            if(messageAll[i].timestamp == this.messages[0].timestamp){
              lastIndex = i
              break
            }
          }
          if(lastIndex == -1) return []
          messages = messageAll.slice(-(messageAll.length - lastIndex + pageSize), lastIndex)
        }
      }
      for(let i = 0; i<messages.length; i++){
        let msg = messages[i]
        msg.isShowCancel = false
        messages[i] = this.handlerMessage(msg)
      }
      return messages
    },
    // 敲键盘发送pong事件消息
    ...
    removeMessage(accountId, key){
       var newMessages = []
        ...
        if(messagesString){
          let messageAll = JSON.parse(messagesString)
          var newCacheMessages = []
          for(let c = 0; c < messageAll.length; c++){
            var msg = messageAll[c]
            if(msg.key == key && msg.from_account == accountId) continue
            if(msg.biz_type == 'welcome' || msg.biz_type == "handshake" || msg.biz_type == "into") continue
            newCacheMessages.push(messageAll[c])
          }
          localStorage.setItem("miniImAppMessageRecord_" + this.userInfo.id, JSON.stringify(newCacheMessages))
        }
